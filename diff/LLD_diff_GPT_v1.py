...
"""
로그 비교 GUI (WinMerge 유사) - diff-match-patch 기반 (정밀판 / GUI 안정화+개선판)

반영한 개선사항(요청 1번)
1) 전역 변수 제거 (threshold/weights를 엔진 파라미터로 전달)
2) 파일 읽기 예외 처리 강화 (PermissionError / OSError 등)
3) TF-IDF 비용 제한 (대용량 본문 샘플링 + 간단 가드)
4) 큐 무한 증가 방지 (maxsize 적용 + progress 이벤트 드롭)
5) Tkinter 스레드 안전성 유지 (워커에서 GUI 직접 접근 금지)
"""
...
# TF-IDF 비용 제한
TFIDF_MAX_CHARS = 50_000  # 본문 50KB(문자 기준)까지만 사용
TFIDF_LEN_RATIO_GUARD = 0.15  # 길이 비율이 너무 벌어지면 유사도 0 처리(비용 절감)
...
IGNORE_LINE_RULES: List[Tuple[re.Pattern, str]] = [
    (re.compile(r"^\s*uid\s*:\s*.*$", re.IGNORECASE), "__IGNORED_UID__"),
    (re.compile(r"^\s*Local time\s*:\s*.*$", re.IGNORECASE), "__IGNORED_LOCAL_TIME__"),
    (re.compile(r"^\s*generation\s*:\s*.*$", re.IGNORECASE), "generation: __IGNORED__"),
    (re.compile(r"^\s*creationTimestamp\s*:\s*.*$", re.IGNORECASE), "creationTimestamp: __IGNORED__"),
    (re.compile(r"^\s*lastTransitionTime\s*:\s*.*$", re.IGNORECASE), "lastTransitionTime: __IGNORED__"),
    (re.compile(r"^\s*lastResync\s*:\s*.*$", re.IGNORECASE), "lastResync: __IGNORED__"),
    (re.compile(r"^\s*observedGeneration\s*:\s*.*$", re.IGNORECASE), "observedGeneration: __IGNORED__"),
    (re.compile(r"^\s*managedFields\s*:\s*.*$", re.IGNORECASE), "managedFields: __IGNORED_SECTION__"),
...
class SimilarityParams:
    threshold: float
    weights: Dict[str, float]  # key/yaml_sig/yaml_keys/tfidf
...
def _yaml_signature_similarity(sig1: Optional[Dict[str, str]], sig2: Optional[Dict[str, str]]) -> float:
    ...
    if sig1.get('kind') == sig2.get('kind'):
        score += 0.3
    if sig1.get('name') and sig2.get('name') and sig1.get('name') == sig2.get('name'):
        score += 0.2
    if sig1.get('namespace') and sig2.get('namespace') and sig1.get('namespace') == sig2.get('namespace'):
        score += 0.1
...
def _body_structure_similarity_tfidf(body1: str, body2: str) -> float:
    ...
    # 비용 가드: 길이가 너무 다르면 TF-IDF 자체를 하지 않음
    l1 = len(body1)
    l2 = len(body2)
    if min(l1, l2) / max(l1, l2) < TFIDF_LEN_RATIO_GUARD:
        return 0.0

    # 샘플링(앞부분만) - 비용 제한
    if l1 > TFIDF_MAX_CHARS:
        body1 = body1[:TFIDF_MAX_CHARS]
    if l2 > TFIDF_MAX_CHARS:
        body2 = body2[:TFIDF_MAX_CHARS]
...
def block_similarity(sb: LogBlock, tb: LogBlock, params: SimilarityParams) -> Tuple[float, Dict[str, float]]:
    # kind mismatch는 매칭 금지
    ...
    combined = (
        key_sim * w.get('key', 0.0) +
        yaml_sig_sim * w.get('yaml_sig', 0.0) +
        yaml_keys_sim * w.get('yaml_keys', 0.0) +
        body_sim * w.get('tfidf', 0.0)
    )
        ...
        if tag == "equal":
            ...
            for si, sb in enumerate(src_span):
                ...
                for tj, tb in enumerate(tgt_span):
                    if tj in used_tgt:
                        continue
                    sim, _ = block_similarity(sb, tb, params)
                    if sim > best_sim:
                        best_sim = sim
                        best_j = tj
            ...
            paired_src = {si for si, _, _ in pairs}
            paired_tgt = {tj for _, tj, _ in pairs}

            for si, tj, _ in sorted(pairs, key=lambda x: x[0]):
                aligned.append((src_span[si], tgt_span[tj], src_span[si].key))
        ...
        *,
        ...
        for rel in matched:
            ...
            if s_lines is None or t_lines is None:
                ...
                advance(f"{rel} (read_error: {s_enc}/{t_enc})")
                continue

            if block_mode:
                src_blocks = split_log_into_blocks(s_lines)
                tgt_blocks = split_log_into_blocks(t_lines)
                aligned = align_blocks(src_blocks, tgt_blocks, self.params)
...
class App:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("로그 비교 GUI (TF-IDF + YAML 키) - 개선판")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(self.opt, text="라인 trim 후 비교", variable=self.trim_var).grid(row=0, column=0, sticky="w", padx=8, pady=4)
        ttk.Checkbutton(self.opt, text="공백만 다른 차이는 무시(연속 공백 정규화)", variable=self.ignore_space_var).grid(row=0, column=1, sticky="w", padx=8, pady=4)
        ttk.Checkbutton(self.opt, text="대소문자 무시", variable=self.ignore_case_var).grid(row=0, column=2, sticky="w", padx=8, pady=4)

        ttk.Checkbutton(self.opt, text="로그 블록(# ... 단위)로 비교 (블록 정렬 후 블록 내부 diff)", variable=self.block_mode_var).grid(
        ...
        ttk.Label(self.opt, text="(직접 입력 가능: 예 foo*.log)").grid(row=2, column=2, sticky="w", padx=8, pady=4)
        ...
        self._log("준비 완료")
        self._log("")
        self._log(self._param_info_text("【 현재 설정 파라미터 】"))
        self._log("")
    ...
    def _param_info_text(self, header: str) -> str:
        ...
        return "\n".join([
            ...
            header,
            "=" * 80,
            f"임계값:              {self.threshold_var.get():.2f} (블록이 이 이상 유사해야 매칭)",
            "",
            "가중치 설정:",
            f"  • 블록 키:        {self.key_weight_var.get():.2f} (이름이 같은가?)",
            f"  • YAML 서명:      {self.yaml_sig_weight_var.get():.2f} (kind가 같은가?)",
            f"  • YAML 키 구조:   {self.yaml_keys_weight_var.get():.2f} (구조가 같은가?)",
            f"  • TF-IDF (본문):  {self.tfidf_weight_var.get():.2f} (내용이 같은가?)",
            "  ─────────────────────",
            f"  합계:             {total:.2f} {check}",
            "=" * 80,
        ])
    ...
    def _safe_put(self, item: Tuple[str, object]) -> None:
        # 큐가 꽉 차면 progress 이벤트는 드롭 (done/error는 드롭되면 안 되므로 호출부에서 구분)
        try:
    ...
    def _make_params(self) -> SimilarityParams:
    ...
    def start(self) -> None:
        ...
        self._log("비교 시작...")
        self._log("")
        self._log(self._param_info_text("【 비교 시작 - 설정된 파라미터 】"))
        self._log("")

        def progress_cb(done: int, total: int, label: str) -> None:
            # progress는 드롭 가능
            self._safe_put(("progress", (done, total, label)))

        def worker() -> None:
            try:
                engine = ComparisonEngine(params=params, progress_cb=progress_cb)
                start_ts = time.time()
    ...
    def _show_done(self, result: Dict[str, object], elapsed: float) -> None:
        ...
        self._log(f"  ✗ 읽기/인코딩 오류: {summary['EncodingError']}")
        ...
        messagebox.showinfo(
            ...
            f"✗ 읽기/인코딩 오류: {summary['EncodingError']}\n\n"
